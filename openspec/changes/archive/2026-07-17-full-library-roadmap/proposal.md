...
- зміни на завантаженій книзі **мовчки губляться**: `ws['A1']=x` не позначає лист
  ...
  старий `sharedString` індекс (D-4) — включно з найпоширенішою openpyxl-ідіомою
  `ws['A1'].value = x`, яка проходить повз обидва мутатори (`value` — публічний
  C-атрибут), тож губиться і як `_modified`, і як shared-string посилання;
...
- типи даних спотворюються: `bool`→`int` (D-7), дати лишаються числами (D-8),
  `inlineStr` губиться у standard/chunked-парсерах (D-9) — а це, як показала
  перевірка, **будь-який великий файл від openpyxl**, бо той пише `t="inlineStr"`
  за замовчуванням;
...
покривають фази 0–2 та **streaming-частину фази 3** (task 5.1, D-10) — найкритичніше
й одразу тестоване. Решта фази 3 (write-only, бенчмарк-набір), фази 4–5 та наукова
новизна включені до плану (`design.md`, `tasks.md`) і отримають власні delta-specs
у наступних змінах, коли будуть детально проскоповані.

Streaming (5.1) втягнуто у формальний scope, бо на нього зʼявився зовнішній патч
(`cytosheet_stream_values_patch.pyx`), а спека `xlsx-io` **вже обіцяє** потокове
читання («без завантаження листа цілком у памʼять»), якого в реалізації немає:
lazy-режим читає XML листа двічі й матеріалізує всі `Cell`. Тут спека переважно
підтягується до чесної реалізації, а не змінюється.
...
- **Фаза 2 — Вірність типів:** `bool`, дати (`serial↔datetime`), `inlineStr` у всіх
  парсерах, значення-помилки. `inlineStr` (D-9) втягнуто у формальний scope разом
  зі streaming: фолбек за відсутності `<dimension>` матеріалізує лист standard-парсером,
  тож без D-9 віддає стовпці `None` (design D9).
...
- `xlsx-io`: коректний мапінг листів через relationships; збереження змін і збереження стилів завантаженої книги при `save`; lazy-режим не читає XML листа при відкритті; `save` працює на read-only книзі (краще за openpyxl) з піком у розмір одного листа; `save` після `close` → `ValueError`; емісія `<dimension>`.
- `cell-model`: вірність типів даних (`bool`/дати/`inlineStr`) та ізоляція стилю кожної комірки.
- `worksheet-operations`: відстеження модифікацій листа (включно з `ws[coord].value = x`); завантаження листа не позначає його зміненим (байтова точність незмінених листів); справжнє потокове читання без матеріалізації з позиціями комірок за `r=` і падінгом розріджених рядків; збереження identity комірок у звичайному режимі.
