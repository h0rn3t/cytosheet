...
- [x] 9.1 `openspec validate --strict full-library-roadmap` зелений
- [x] 9.2 Усі тести фаз 0–2 проходять; немає регресій у наявних тестах (95 passed)
- [x] 9.3 Review summary (нижче); `openspec archive full-library-roadmap`

## Review summary

**Що закрито:** фази 0–2 повністю + streaming-частина фази 3 (task 5.1) — тобто весь
формальний scope зміни. Дефекти D-1…D-10 усунено, кожен має тест, що його відтворює.
Тестів: 95 (було 78 на старті фази 3). Гілка `full-library-roadmap-phase3-streaming`.

**Ключові результати:**
- Стримінг тримає сталу памʼять: 25.5 → 26.8 MB (+5.2%) за 20× даних; матеріалізуючий
  шлях на тому самому файлі — 86.6 → 1292 MB (+1392%). **48× виграш на 200k рядків.**
- Дати/bool/помилки/inlineStr звірені з openpyxl як ground truth в обидва боки.
- Байтова точність незмінених листів збережена (COMPAT-2) — під regression-тестом.

**Trade-offs і рішення, прийняті по ходу:**
1. **Окремий `stream_values()` відхилено** (design D6.1) на користь `iter_rows`: openpyxl
   стримить саме через нього, окреме імʼя порушило б COMPAT-3. Task 5.1.10 лишається
   невиконаним свідомо — це однорядковий аліас, якщо імʼя знадобиться зовні.
2. **Два шляхи ітерації** (lazy стримить, eager іде bbox-шляхом) — ризик дрейфу прийнято
   заради identity комірок при записі; мітигація — спільні `_value_from_t`/`_maybe_date`
   і тест паритету обох шляхів на одному файлі.
3. **Фолбек на матеріалізацію без `<dimension>`** — коректність над памʼяттю (design D6.4).
4. **`Cell.value` → property** (design D8): ризик перезбірки й просідання. Просідання
   виміряно й НЕ підтвердилось (31.8 → 31.4 ns). Ціна, що лишилась: конструктор `Cell`
   мусить лишатись «німим» (пише `_value` повз сеттер), інакше парсер позначить кожен
   лист зміненим і COMPAT-2 помре. Це неочевидно — прикрито тестом
   `test_load_and_save_unmodified_is_byte_identical`.

**Розширення scope по ходу (обидва — блокери, не бажанки):**
- **4.3 (inlineStr)** втягнуто з фази 2, бо фолбек 5.1.7 матеріалізує standard-парсером
  і без нього віддавав стовпці `None`. Виявилось ширше за опис у CLAUDE.md §10: openpyxl
  пише `t="inlineStr"` за замовчуванням, тож дефект бив по будь-якому великому
  openpyxl-файлу, а не лише «≥50 KB».
- **4.6 (XML-сутності)** — знайдено крос-тестом 4.5, не було в жодному плані: простий
  парсер не декодував сутності, `'a & b'` читався як `'a &amp; b'`. Тиха порча даних на
  дефолтному шляху малих файлів.

**Спостереження для наступних змін:**
- Тричі поспіль корінь бага був той самий: **логіка, продубльована між трьома парсерами**
  (вилучення значення → `_cell_from_element`; диспетчеризація за `t=` → `_value_from_t`;
  type inference → `set_value`). CS-7 варто підняти з SHOULD до MUST.
- **Неповні довідкові таблиці = тиха втрата даних.** `BUILTIN_NUMFMTS` мала 9 записів
  із 36, через що час читався числом. Такі таблиці або повні, або їх нема.
- `print()`/`except Exception: pass` у парсерах (D-12, task 5.3) досі на місці — під час
  дебагу дат вони глушили помилки й заважали. Кандидат №1 у наступну зміну.

**Свідомо не зроблено (поза формальним scope, окремими змінами):** 5.1.10, 5.2 (write-only),
5.3 (D-12), 5.4 (бенчмарк-набір), фази 4–5, наукова новизна. Roadmap лишається в CLAUDE.md §11.
