...
cdef class Worksheet:
    ...
    cpdef void _parse_sheet(self, bytes xml_data):
        ...
        cdef object context = etree.iterparse(io.BytesIO(xml_data), events=("end",))
        cdef object event
        cdef object elem
        cdef str col_ref
        cdef str cell_type
        ...
        for event, elem in context:
            if elem.tag == NS_MAIN + 'mergeCell':
                col_ref = elem.attrib.get('ref')
                if col_ref:
                    self._merged_cells.add(col_ref)
                elem.clear()
            elif elem.tag == NS_MAIN + 'c':
